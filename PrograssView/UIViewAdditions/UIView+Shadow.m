...
+(UIView*)tableviewHead:(float)radius Color:(UIColor *)color frame:(CGRect)frame{
    UIView* tempv=[[UIView alloc]initWithFrame:CGRectMake(0, 0, frame.size.width, radius)];
    tempv.backgroundColor=[UIColor clearColor];
    
    CAGradientLayer *gradient = [CAGradientLayer layer];
    gradient.frame = tempv.bounds;
    gradient.colors = [NSArray arrayWithObjects:(id)[[color colorWithAlphaComponent:0.5] CGColor], (id)[[UIColor clearColor] CGColor], nil];
    [tempv.layer insertSublayer:gradient atIndex:0];
